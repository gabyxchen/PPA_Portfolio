...
```{r}
# Add a age_MOE__percentage column calculation
county_reliability <- county_reliability %>%
  mutate(age_MOE_percentage = median_ageM / median_ageE * 100)
# Scatter plot
ggplot(data = county_reliability, 
       aes(x = total_popE, 
           y = age_MOE_percentage)) +
  ...
  scale_x_continuous(labels = comma)
  labs(
    title = "Data Reliability by Population Size",
    x = "Total Population",
    y = "Margin of Error (%)"
  ) +
  theme_classic()
...
# Box plots by population category
ggplot(county_reliability) +
  aes(x = pop_category, y = age_MOE_percentage, fill = pop_category) +
  geom_boxplot() +
  labs(
    title = "Data Reliability by Population Size Category",
    x = "Total Population",
    y = "Margin of Error (%)"
  ) +
  theme_classic() +
...
```{r}
# Create income categories and faceted plot
county_faceted <- county_data %>%
  mutate(
    income_category = case_when(
      median_incomeE < 60000 ~ "Low (< 60000)",
      median_incomeE < 75000 ~ "Medium (60000-75000)",
      TRUE ~ "High (75000+)"
    )
  )

ggplot(county_faceted) +
  aes(x = total_popE, y = median_ageE) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~income_category) +
  labs(
    title = "Population-Age Relationship by Median Income",
    x = "Total Population",
    y = "Median Age"
  ) +
  theme_minimal() +
...
education_data <- get_acs(
  ...
  mutate(
    ...
    county_name = str_remove(NAME, paste0(", ", "Pennsylvania"))
  ) %>%
