...
burglaries <- st_read(here("lecture/week9", "burglaries.shp")) %>% 
  st_transform('ESRI:102271')
...
*Your answer here: 7482; 2017-2018; It allows accurate distance calculations.*
...
*Your answer here:* In 2017, burglaries clustered in both northern and southern Chicago. The highest concentration is in southeast Chicago. There are a lot of possible influences to these patterns, including the policing spatial bias, racial bias, unreported incidents, and trust in police department, etc.
...
*Your answer here:* Fishnet controls the size of the analysis units. The advantags of fishnet approach are that it's a standardized approach in predictive policing and makes the spatial operations easier. The disadvantages are that it is arbitrary, and that it might split existing administrative and natural boundaries.
...
*Your answer here:* There are a lot of zero counts and few high counts. Small cell sizes may increase the number of zero count cells. This distribution is suitable for a count regression model but not a basic Poisson model.
...
*Your answer here:* KDE map has a much smaller variance than the count map. It captures
...
abandoned_cars <- read_csv("abandoned_cars_2017.csv") %>%
